...
{
    public class vParentCreateLoginRepository : GenericRepository<vParentForCreateLoginID>
    {
        public vParentCreateLoginRepository() : base(new dbSchoolAppEntities()) { }
        public vParentCreateLoginRepository(dbSchoolAppEntities dbContext) : base(dbContext) { }



        public List<vParentForCreateLoginID> GetParentListForCreateLoginID(byte mCompID, byte mBranchID)
        {
            List<vParentForCreateLoginID> obj = new List<vParentForCreateLoginID>();
            obj = this.context.vParentForCreateLoginIDs.Where(x => x.CompID == mCompID && x.BranchID == mBranchID).ToList();
            return obj;
        }


        public vParentForCreateLoginID GetStudentDetailForLogin(int StudentID, byte BranchID, byte CompID)
        {

            vParentForCreateLoginID obj = new vParentForCreateLoginID();
            obj = this.context.vParentForCreateLoginIDs.Where(x => x.StudentID == StudentID && x.BranchID == BranchID && x.CompID == CompID).FirstOrDefault();
            return obj;
        }
