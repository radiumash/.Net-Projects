...
{
    public class vTeacherCreateLoginRepository : GenericRepository<vTeacherForLoginID>
    {
        public vTeacherCreateLoginRepository() : base(new dbSchoolAppEntities()) { }
        public vTeacherCreateLoginRepository(dbSchoolAppEntities dbContext) : base(dbContext) { }



        public List<vTeacherForLoginID> GetTeacherListForCreateLoginID(byte mCompID, byte mBranchID)
        {
            List<vTeacherForLoginID> obj = new List<vTeacherForLoginID>();
            obj = this.context.vTeacherForLoginIDs.Where(x => x.CompID == mCompID && x.BranchID == mBranchID).ToList();
            return obj;
        }


        public vTeacherForLoginID GetTeacherDetailForLogin(int TeacherID, byte BranchID, byte CompID)
        {

            vTeacherForLoginID obj = new vTeacherForLoginID();
            obj = this.context.vTeacherForLoginIDs.Where(x => x.TeacherID == TeacherID && x.BranchID == BranchID && x.CompID == CompID).FirstOrDefault();
            return obj;
        }
